...
public class AdvancedPlayerMovement2D : MonoBehaviour
{
    ...
    void Update()
    {
        ...
        // O input de movimento é lido, mas a física só o usará se não estiver em um estado de ação.
        moveInput = Input.GetAxisRaw("Horizontal");

        // A lógica de virar agora funciona mesmo durante o dash.
        if (!isWallSliding && !isWallJumping) HandleFlipLogic();
    ...
    {
        // Vira apenas com base no input de movimento.
    ...
    private void CheckCollisions() { Vector2 capsuleCenter = (Vector2)transform.position + capsuleCollider.offset; Vector2 capsuleSize = capsuleCollider.size; RaycastHit2D hit = Physics2D.CapsuleCast(capsuleCenter, capsuleSize, capsuleCollider.direction, 0f, Vector2.down, 0.1f, collisionLayer); isGrounded = hit.collider != null && Vector2.Angle(hit.normal, Vector2.up) < 45f; if (isGrounded) { isWallJumping = false; coyoteTimeCounter = coyoteTime; } float wallRayStartOffset = capsuleCollider.size.x * 0.5f; isTouchingWallRight = Physics2D.Raycast(capsuleCenter, Vector2.right, wallRayStartOffset + wallCheckDistance, collisionLayer); isTouchingWallLeft = Physics2D.Raycast(capsuleCenter, Vector2.left, wallRayStartOffset + wallCheckDistance, collisionLayer); }
    ...
    public void StartWallSlide() { isWallSliding = true; rb.linearVelocity = new Vector2(0, 0); if ((isTouchingWallRight && isFacingRight) || (isTouchingWallLeft && !isFacingRight)) { Flip(); } }
    public void DoWallJump(float multiplier) { isWallSliding = false; isWallJumping = true; Vector2 ejectDirection = GetWallEjectDirection(); rb.linearVelocity = new Vector2(ejectDirection.x * wallJumpForce.x, wallJumpForce.y * multiplier); Flip(); }
